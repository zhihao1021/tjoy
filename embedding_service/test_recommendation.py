...
# -*- coding: utf-8 -*-
"""
單元測試：測試 ActivityRecommendationSystemGemma 推薦系統
創建 10 個活動和 5 個使用者進行測試
"""
...
from model.search_history import SearchHistoryModel
...
class TestRecommendationSystem(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 載入模型（在實際測試中可能需要模擬）
        try:
            self.recommendation_system.load_model()
        except Exception as e:
            print(f"模型載入失敗，跳過需要模型的測試: {e}")
            self.skipTest("模型載入失敗")
    ...
    def _create_users(self):
        ...
        for data in user_data:
            ...
            user.follow_categories = [cat for cat in self.categories if cat.name in data["interests"]]
    ...
    def test_recommend_events_success(self):
        """測試成功推薦活動"""
        # 設定使用者
        self.recommendation_system.set_user(self.users[0])  # 愛麗絲（程式設計興趣）
        ...
        # 獲取推薦
        recommended_event_ids = self.recommendation_system.recommend_events(top_k=3)
        
        # 驗證結果
        self.assertIsInstance(recommended_event_ids, list)
        self.assertEqual(len(recommended_event_ids), 3)
        self.assertTrue(all(isinstance(event_id, str) for event_id in recommended_event_ids))
        
        # 驗證推薦的活動確實存在
        for event_id in recommended_event_ids:
            self.assertIn(event_id, self.recommendation_system.events_data)
    
    def test_recommend_events_different_users(self):
        """測試不同使用者的推薦結果"""
        # 測試愛麗絲（程式設計興趣）
        self.recommendation_system.set_user(self.users[0])
        for i, event in enumerate(self.events):
        ...
        alice_recommendations = self.recommendation_system.recommend_events(top_k=5)
        
        # 測試鮑伯（音樂興趣）
        self.recommendation_system.set_user(self.users[1])
        bob_recommendations = self.recommendation_system.recommend_events(top_k=5)
        
        # 驗證不同使用者有不同的推薦結果
        self.assertNotEqual(alice_recommendations, bob_recommendations)
        
        # 驗證推薦結果都是有效的活動 ID
        for event_id in alice_recommendations + bob_recommendations:
            self.assertIn(event_id, self.recommendation_system.events_data)
    
    def test_user_embedding_caching(self):
        """測試使用者嵌入向量快取"""
        user = self.users[0]
        self.recommendation_system.set_user(user)
        
        # 第一次計算
        embedding1 = self.recommendation_system.create_user_profile_embedding()
        self.assertIsNotNone(self.recommendation_system.user_embedding)
        
        # 第二次計算應該使用快取
        embedding2 = self.recommendation_system.create_user_profile_embedding()
    ...
    def test_comprehensive_score_calculation(self):
        """測試綜合分數計算"""
        # 設定使用者和活動
        self.recommendation_system.set_user(self.users[0])  # 愛麗絲
        self.recommendation_system.add_event_data("event_1", self.events[0])  # Python 工作坊
...
def run_tests():
    """執行測試"""
    # 創建測試套件
    test_suite = unittest.TestLoader().loadTestsFromTestCase(TestRecommendationSystem)
